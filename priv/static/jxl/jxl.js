(function () {
  ...
  async function loop() {
    ...
    if (document.body.contains(job[1])) {
      const res = await fetch(job[0])
      await process(res, job[1], job[2], job[3])
    }
  ...
  function addToQueue(jxlSrc, img, isCSS, isSource) {
    queue.push([jxlSrc, img, isCSS, isSource])
    setTimeout(loop, 0)
  }

  function check(el) {
    [...el.childNodes].reverse().forEach(n => check(n))
  ...
  function imgDataToDataURL(img, imgData, isCSS, isSource, complete=false) {
    ...
    if (imgData instanceof Blob) {
      ...
      canvas.toBlob(blob => {
        ...
        complete && config.useCache && cache && cache.put(jxlSrc, new Response(blob))
  ...
  async function decode(img, isCSS, isSource) {
    ...
    addToQueue(jxlSrc, img, isCSS, isSource)
  }
  ...
  async function process(res, img, isCSS, isSource) {
    ...
    function processChunk(chunkLen) {
      ...
      if (result.copyPixels) {
        ...
        imgDataToDataURL(img, imgData, isCSS, isSource, result.complete)
      }
      return result
    }
  ...
  script.src = isSimd ? '/jxl/jxl_decoder_simd.js' : '/jxl/jxl_decoder.js'
  script.onload = init
